...
Often the nwis that comes from long records of stream observation are noisy. The records of discharge, width, depth and velocity from gauges although being extremely valuable require careful consideration, and flittering due to reasons including but not limited to:
...
Here we will illustrate the packaged methods. An assumption of _all_ filtering methods is that if it reduces the nwisset to less then 10 observations, a warning is emitted and the input nwis is returned. The initial step in filtering is to screened for any negative, inf values. Negative value in observations is often associated with gauges that are located in backwater zones and are affected by the coastal processes including sea level changes, tides and storm surge that can extend hundreds of kilometers inland.
