...
#' @param keep_max flag indicating if the Should the largest flow record be kept, even if older then "years"
#' @return data.frame
#' @family filters
#' @export
...
mad_filter = function(df, envelope = 3) {
  ...
  if (nrow(df2) < 10) {
    warning(
      "Too much data was filtered using MAD. Less then 10 obs remain. Keeping complete history.",
      call. = FALSE
    )
  } else {
    df = df2
...
nls_filter = function(df, allowance = .5) {
  ...
  if (nrow(df2) < 10) {
    warning(
      "Too much data was filtered using NLS window. Less then 10 obs remain. Keeping all data.",
      call. = FALSE
    )
  } else {
    df = df2
