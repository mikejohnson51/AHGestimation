...
calc_ga = function(Q, Y, V, TW, allowance = .05, r, type = "nrmse"){

if(type == "nrmse"){
  fitness <- function(x) {
      ## order: k,m,a,b,c,f
      v = hydroGOF::rmse(x[1]*Q^x[2], V) / mean(V)
      t = hydroGOF::rmse(x[3]*Q^x[4], TW) / mean(TW)
      d = hydroGOF::rmse(x[5]*Q^x[6], Y) / mean(Y)
      return(c(v,t,d))
    }
} else {
...
cont = function(par){
  data.frame(id = 1:nrow(par),
             r = par[,6] / par[,4],
             c1 = par[,1] * par[,3] * par[,5],
             c2 = par[,2] + par[,4] + par[,6]) %>% 
    mutate(c1_e = abs(1-c1), 
           c2_e = abs(1-c2), 
           c3 = c1_e + c2_e, rank = rank(c3))
}
...
v = hydroGOF::rmse(vp, V) / mean(V)
t = hydroGOF::rmse(tp, TW) / mean(TW)
d = hydroGOF::rmse(dp, Y) / mean(Y)

v2 = hydroGOF::pbias(vp, V)
t2 = hydroGOF::pbias(tp, TW)
d2 = hydroGOF::pbias(dp, Y)
...
r$Y[3,] = data.frame(type = "Y", exp = min[6], coef = min[5], 
                     nrmse = d, pb = d2, mean_ape = d3, min_ape = d4, max_ape = d5,
                     method = "GA", stringsAsFactors = FALSE)
r$TW[3,] = data.frame(type = "TW", exp = min[4] , coef = min[3],
                      nrmse = t, pb = t2, mean_ape = t3, min_ape = t4, max_ape = t5, 
                      method = "GA", stringsAsFactors = FALSE)
r$V[3,]  = data.frame(type = "V", exp = min[2] , coef = min[1], 
                      nrmse = v, pb = v2, mean_ape = v3, min_ape = v4, max_ape = v5,
                      method = "GA", stringsAsFactors = FALSE)
