...
#' @param scale Scale by set factor.
#' This limits the exponent at coefficients to the range of 
#' {(1/s) * nls; s * nls}
#' @return list
#' @family AHG
#' @export

min_max <- function(df, scale = 2) {
  ...
  if (is.null(scale)) {
    ...
    o <- filter(compute_ahg(df$Q, df$TW, "TW"), method == "nls")
      TW_e_l <-  (1 / scale) * o$exp
      TW_e_h <-  scale     * o$exp
      TW_c_l <- (1 / scale) * o$coef
      TW_c_h <- scale     * o$coef
      TW_e     <- o$nrmse
      TW_e_nls <- o$exp
      TW_c_nls <- o$coef
...
run_it <- function(fitness, 
  ...
  errors <- function(vals) {
    ...
    vals %>% 
      data.frame() %>%
      mutate(id = seq(nrow(vals)), error = rowSums(vals)) %>%
...
#' Percent Bias
#' @description Percent Bias between sim and obs, 
...
#' @description Normalized root mean square error (NRMSE) between sim and obs, 
#' with treatment of missing values
#' @param sim numeric vector simulated values
#' @param obs numeric vector observed values
#' @return numeric
...
nrmse <- function(sim, obs) {
  ...
  # index of those elements that are present both 
  # in 'sim' and 'obs' (NON- NA values)
  vi <- valindex(sim, obs)
...
valindex <- function(sim, obs) {  
    ...
    if (length(index) == 0) {
      warning(paste0("'sim' and 'obs' are empty or they",
                     "do not have any common pair of elements with data!")
      )
    }
    return( index  )
} 
...
calc_nsga <- function(df, 
  ...
  o2 <- do.call('rbind', o2)
  # https://ai.stackexchange.com/questions/12019/
  # how-to-find-optimal-mutation-probability-and-crossover-probability
  #   o2 = rbind(# high crossover, low mutation
  #         run_it(fitness, c4, 1, .8, .05, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         run_it(fitness, c4, 4, .8, .05, 
  #                lower.bounds = mm$low,  upper.bounds = mm$high),
  #         run_it(fitness, c4, 7, .8, .05, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         
  #         # high-ish crossover, low-ish mutation
  #         run_it(fitness, c4, 2, .6, .2, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         run_it(fitness, c4, 5, .6, .2, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         run_it(fitness, c4, 8, .6, .2, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         
  #         # moderate crossover, moderate mutation
  #         run_it(fitness, c4, 3, .4, .4, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         run_it(fitness, c4, 6, .4, .4, 
  #                lower.bounds = mm$low, upper.bounds = mm$high),
  #         run_it(fitness, c4, 9, .4, .4, 
  #                lower.bounds = mm$low, upper.bounds = mm$high)
  #   )
  # })
